<div class="px-4">
    ...
    <div class="container m-auto text-center">
      <h1 class="my-4 mt-10 text-4xl text-center text-green-600 md:mt-0 md:text-5xl"><b>{{$step == 1 ? 'Your name is important for us' : 'Acces a group'}}</b></h1>
      <div class="w-full pt-10 m-auto mt-10">
        <div class="w-full max-w-sm m-auto mt-10 mx:mt-0" x-data="{step: @entangle('step')}">
            ...
            <form wire:submit.prevent="start" x-show="step == 2" class="flex items-center py-2 border-b border-teal-500">
                <input wire:model='roomCode' class="w-full px-2 py-1 mr-3 leading-tight text-gray-700 bg-transparent border-none appearance-none focus:outline-none" type="text" placeholder="Group Code ...">
                <button type="submit" class="flex-shrink-0 px-2 py-1 text-sm text-white bg-teal-500 border-4 border-teal-500 rounded hover:bg-teal-700 hover:border-teal-700" type="button">
                    Start
                </button>
                <a href="/" class="flex-shrink-0 px-2 py-1 text-sm text-teal-500 border-4 border-transparent rounded hover:text-teal-800" type="button">
            ...
            @error('roomCode') <span class="text-red-600 error">{{ $message }}</span> @enderror
            <span class="text-red-600 error">{{ $messageErr }}</span>
        </div>
      </div>
    </div>
  </div>
